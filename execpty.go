...
func runProcessViaPTY() (int, error) {
	...
	// что если считываемое сообщение такое же как то, что лежит в канале, выводить его не будет.
	messageChan := make(chan string, 1)

	wg := &sync.WaitGroup{}
	wg.Add(1)

	// Начинмем в отдельном потоке считывать сканером псевдотерминал, чтобы
